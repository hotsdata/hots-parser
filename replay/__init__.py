...
def _normalize_protocol_value(value):
    ...
    if isinstance(value, tuple):
        return tuple(_normalize_protocol_value(item) for item in value)
    if isinstance(value, dict):
        return {_normalize_protocol_value(key): _normalize_protocol_value(item) for key, item in value.items()}
    return value
...
class Replay:
    ...
    def get_battle_tags(self, lobby_data, player):
        lobby_data = _to_text(lobby_data)
        tag = ""
    ...
    def process_replay_details(self):
        ...
        self.protocol, protocol_build, protocol_fallback = get_protocol_for_build(baseBuild)
        self.details = _normalize_protocol_value(self.protocol.decode_replay_details(contents))
        self.replayInfo = HeroReplay(self.details)
        self.replayInfo.gameVersion = baseBuild
        self.replayInfo.protocolBuild = protocol_build
        self.replayInfo.protocolFallback = protocol_fallback
    ...
    def process_replay_initdata(self):
        ...
        initdata = _normalize_protocol_value(self.protocol.decode_replay_initdata(contents))
        ...
        for p in range(0, len(initdata["m_syncLobbyState"]["m_lobbyState"]["m_slots"])):
            slotData = initdata["m_syncLobbyState"]["m_lobbyState"]["m_slots"][p]
            for i in self.players:
                if self.players[i].toonHandle == _to_text(slotData["m_toonHandle"]):
    ...
    def process_replay_attributes(self):
        ...
        attributes = _normalize_protocol_value(self.protocol.decode_replay_attributes_events(contents))

        # Get if players are human or not
        for playerId in list(attributes["scopes"].keys()):
        ...
        if not self.replayInfo.gameType:
            ...
            if attributes["scopes"][16][3009][0]["value"] == "Amm":
                if attributes["scopes"][16].get(4010, None) is not None:
                    if (
                        attributes["scopes"][16][4010][0]["value"] == "drft"
                        and attributes["scopes"][16][4018][0]["value"] == "pred"
                    ):
                        gameType = "Hero League"
                    elif (
                        attributes["scopes"][16][4010][0]["value"] == "drft"
                        and attributes["scopes"][16][4018][0]["value"] == "fcfs"
                    ):
                        gameType = "Team League"
                    else:
                        gameType = "Quick Match"
            elif (
                attributes["scopes"][16][3009][0]["value"] == "Priv"
                and attributes["scopes"][16][3009][0]["value"] == "hero"
            ):
                gameType = "Custom Game"
    ...
    def get_lifespan_time_in_gameloops(self, unitTag):
        return (
            self.unitsInGame[unitTag].gameLoopsAlive
    ...
    def get_lifespan_time_in_seconds(self, unitTag):
        return (
            get_seconds_from_int_gameloop(self.unitsInGame[unitTag].gameLoopsAlive)
            if self.unitsInGame[unitTag].gameLoopsAlive >= 0
    ...
    def get_clicked_units(self):
        return [unit for unit in self.unitsInGame.values() if len(unit.clickerList) > 0]
    ...
    def process_tinker_stats(self):
        ...
        for scrap in [
            self.unitsInGame[u] for u in self.unitsInGame if "TinkerSalvageScrap" == self.unitsInGame[u].internalName
        ]:
            if scrap.was_picked():
    ...
    def process_butcher_stats(self):
        ...
        for meat in [
            self.unitsInGame[u]
            for u in self.unitsInGame
            if "ButcherBloodGlobeFreshMeat" == self.unitsInGame[u].internalName
        ]:
            if meat.was_picked():
    ...
    def process_lunara_stats(self):
        for wisp in [
            self.unitsInGame[u] for u in self.unitsInGame if "DryadWispUnit" == self.unitsInGame[u].internalName
        ]:
    ...
    def process_regen_globes_stats(self):
        ...
        :return:
        """
        for capturedUnitTag in list(self.unitsInGame.keys()):
    ...
    def process_clicked_unit(self, e):
        ...
        if unitTag in list(self.unitsInGame.keys()):
            # Process Tributes
            ...
            if self.heroList[hero].generalStats["deathCount"] > 0:
                ...
                for death in self.heroList[hero].generalStats["deaths"]:
                    ...
                    for allied in allies:
                        ...
                        positions = get_position_by_second(
                            self.unitsInGame[alliedTag], duration, seconds - 15, seconds + 15
                        )
                        # get the positions for the allied player in a window of 6 seconds
                        for s in range(seconds - 3, seconds + 3):
                            if positions.get(s):
                        ...
                        if w_dist is not None and w_dist >= 10:
    ...
    def process_tomb_of_the_spider_queen(self):
        for unitTag in list(self.unitsInGame.keys()):
            if self.unitsInGame[unitTag].is_tomb_of_the_spider_pickable():
                ...
                # process picked souls
                    ...
                    team = self.unitsInGame[unitTag].team
                    self.teams[team].mapStats["pickedSoulGems"] += TOMB_OF_THE_SPIDER_PICKABLE[
                        self.unitsInGame[unitTag].internalName
                    ]
            # process spider boss
            ...
            # get how many structures died in the lane the spider was
                ...
                for unit in list(self.unitsInGame.keys()):
                    ...
                    diedAt = (
                        self.unitsInGame[unitTag].diedAtGameLoops
                        if self.unitsInGame[unitTag].diedAtGameLoops is not None
                        else self.replayInfo.gameLoops
                    )
                    if (
                        targetDiedAt in range(bornAt, diedAt + 1)
                        and targetDiedY in range(spiderY - 20, spiderY + 21)
                        and self.unitsInGame[unit].team != team
                    ):
                        if self.unitsInGame[unit].is_building():
                            self.teams[team].mapStats["totalBuildingsKilledDuringSpiders"] += 1
                        elif (
                            self.unitsInGame[unit].is_hired_mercenary()
                            or self.unitsInGame[unit].is_army_unit()
                            or self.unitsInGame[unit].is_advanced_unit()
                        ):
        ...
        for unitTag in list(self.unitsInGame.keys()):
            ...
            if unit.internalName in GHOST_SHIP:
                for team, when, duration in unit.ownerList:
                    ...
                    if team in range(0, len(self.teams)):
                        effectiveness = 0
                        units_killed = 0
                        buildings_destroyed = 0
                        for u in list(self.unitsInGame.keys()):
                            enemy = self.unitsInGame[u]
                            if enemy.diedAtGameLoops is not None:
                                if (
                                    enemy.team != unit.team
                                    and enemy != unit
                                    and enemy.get_death_time(self.replayInfo.duration_in_secs())
                                    in range(when, when + duration + 1)
                                    and enemy.isDead
                                ):
    ...
    def process_infernal_shrines(self):
        for unitTag in list(self.unitsInGame.keys()):
            if self.unitsInGame[unitTag].is_punisher():
                ...
                died_at = (
                    self.unitsInGame[unitTag].diedAt
                    if self.unitsInGame[unitTag].diedAt > 0
                    ...
                    for unit in list(self.unitsInGame.keys()):
                        if self.unitsInGame[unit].diedAtGameLoops is not None:
                            if (
                                ...
                                and unit != unitTag
                                and self.unitsInGame[unit].get_death_time(self.replayInfo.duration_in_secs())
                                in range(created_at, died_at + 1)
                                ...
                                if (
                                    ...
                                    or self.unitsInGame[unit].is_army_unit()
                                    or self.unitsInGame[unit].is_advanced_unit()
                                    or self.unitsInGame[unit].is_building()
                                ):
    ...
    def process_dragon_shire(self):
        ...
        dragon_creation_time_sorted = sorted(list(dragon_creation_time.keys()), key=lambda s: s)

        for upgrade in list(self.upgrades.keys()):
            if self.upgrades[upgrade].is_dragon_upgrade():
                ...
                contested_time = sum(
                    [
                        (dur or 0)
                        for team, when, dur in dragon_unit.ownerList
                        if team == -1 and when not in dragon_creation_time
                    ]
                )
                wasted_dragon_time_t0 = sum(
                    [
                        (dur or 0)
                        for team, when, dur in dragon_unit.ownerList
                        if team == 0 and when not in dragon_creation_time
                    ]
                )
                wasted_dragon_time_t1 = sum(
                    [
                        (dur or 0)
                        for team, when, dur in dragon_unit.ownerList
                        if team == 1 and when not in dragon_creation_time
                ...
                try:
                    for unit in list(self.unitsInGame.keys()):
                        if self.unitsInGame[unit].diedAtGameLoops is not None:
                            ...
                            if (
                                victimTeam != dragonTeam
                                and unit != dragon_unit.unitTag
                                and victimDeathTime in range(dragon_created_at, diedAt + 1)
                                and self.unitsInGame[unit].isDead
                                ...
                                if distance_from_dragon > 0:
                                    dragon_effectiveness += (
                                        10 / distance_from_dragon * self.unitsInGame[unit].get_strength()
                                    )
                                if self.unitsInGame[unit].is_building():
                                    buildings_killed_during += 1
                                else:
    ...
    def process_garden_of_terror(self):
            ...
            if len(self.unitsInGame[unitTag].ownerList) > 0:  # if the unit was controlled
                ...
                try:
                    for unit in list(self.unitsInGame.keys()):
                        if self.unitsInGame[unit].diedAtGameLoops is not None:
                            if (
                                self.unitsInGame[unit].team != self.heroList[controller_playerId].team
                                and unit != unitTag
                                and self.unitsInGame[unit].get_death_time(self.replayInfo.duration_in_secs())
                                in range(bornAt, diedAt + 1)
                                and self.unitsInGame[unit].isDead
                            ):
                                if (
                                    self.unitsInGame[unit].is_hired_mercenary()
                                    ...
                                    if distance_from_plant > 0:
                                        plant_effectiveness += (
                                            10 / distance_from_plant * self.unitsInGame[unit].get_strength()
                                        )
                                        # that 10 is just a made up number LOL
                                    if self.unitsInGame[unit].is_building():
                                        buildings_killed_during += 1
                ...
                # Update teams stats
                if self.unitsInGame[unitTag].team in range(0, len(self.teams)):
        ...
        for unitTag in [p for p in self.unitsInGame if self.unitsInGame[p].is_plant_pot()]:
            ...
            if plantOwnerTeam in range(0, len(self.teams)):
                potDuration = (
                    pot.gameLoopsAlive if pot.gameLoopsAlive > -1 else (self.replayInfo.gameLoops - pot.bornAtGameLoops)
                )
                ...
                if self.unitsInGame[unitTag].killerPlayerId in range(0, 9):
                    self.teams[abs(team - 1)].mapStats["totalPlantPotsKilled"] += (
                        1  # abs(team - 1) because is rival team
                    )
                    if self.unitsInGame[unitTag].killerPlayerId != self.unitsInGame[unitTag].controlPlayerId:
                        self.heroList[self.unitsInGame[unitTag].killerPlayerId - 1].mapStats[
                            "totalPlantPotsKilled"
                        ] += 1
        ...
        for unitTag in list(self.unitsInGame.keys()):
            if self.unitsInGame[unitTag].is_golem_body():
                ...
                if self.unitsInGame[unitTag].bornAtX == 0 and self.unitsInGame[unitTag].bornAtY == 0:
                    position = [
                        gl
                        for gl in list(golem_body_positions.keys())
                        if gl <= self.unitsInGame[unitTag].bornAtGameLoops
                    ]
                    if len(position) > 0:
                ...
                if diedAt == self.replayInfo.duration_in_secs():
                    ...
                    last_known_position = list(self.unitsInGame[unitTag].positions.keys())[-1]
                ...
                distance_traveled = calculate_distance(
                    self.unitsInGame[unitTag].bornAtX,
    ...
    def get_unit_destruction(self, e):
        ...
        if e["_event"] == "NNet.Replay.Tracker.SUnitDiedEvent":
            ...
            self.unitsInGame[deadUnitTag].gameLoopsAlive = (
                self.unitsInGame[deadUnitTag].diedAtGameLoops - self.unitsInGame[deadUnitTag].bornAtGameLoops
            )
            ...
            if self.unitsInGame[deadUnitTag].killerPlayerId in range(0, len(self.heroList) + 1):
                killer = (
                    self.unitsInGame[deadUnitTag].killerPlayerId - 1
                    if self.unitsInGame[deadUnitTag].killerPlayerId > 0
                    else self.unitsInGame[deadUnitTag].killerPlayerId
                )
            ...
            if self.unitsInGame[deadUnitTag].is_plant_vehicle():
                if len(self.unitsInGame[deadUnitTag].ownerList) > 0:
                    self.unitsInGame[deadUnitTag].ownerList[0][2] = (
                        self.unitsInGame[deadUnitTag].diedAt - self.unitsInGame[deadUnitTag].ownerList[0][1]
                    )
                else:
    ...
    def units_in_game(self):
        return iter(self.unitsInGame.values())

    def heroes_in_game(self):
        return iter(self.heroList.values())
    ...
    def calculate_army_strength(self):
        ...
        for unit in self.units_in_game():
            if unit.team in range(0, len(self.teams)) and (
                unit.is_army_unit() or unit.is_hired_mercenary() or unit.is_advanced_unit()
            ):
                end = unit.get_death_time(self.replayInfo.duration_in_secs())
                for second in range(unit.bornAt, end + 1):
                    try:
    ...
    def process_golem_spawned(self, event):
        ...
        self.timeLine.append({"event": "golemSpawned", "seconds": seconds, "team": team, "skullCount": skullsCollected})
    ...
    def process_dragon_activated(self, event):
        ...
        self.timeLine.append({"event": "dragonActivated", "seconds": seconds, "team": team})
    ...
    def process_immortal_defeated(self, event):
        ...
        self.timeLine.append({"event": "immortalDefeated", "seconds": seconds, "team": team, "power": immortalPower})
    ...
    def process_garden_terror_activated(self, event):
        ...
        self.timeLine.append({"event": "terrorActivated", "seconds": seconds, "team": team})
    ...
    def process_altar_captured(self, event):
        ...
        self.timeLine.append({"event": "altarCaptured", "seconds": seconds, "team": team, "shots": towersCaptured})
    ...
    def process_town_captured(self, event):
        ...
        self.timeLine.append({"event": "fortCaptured", "seconds": seconds, "team": team})
    ...
    def process_tribute_collected(self, event):
        ...
        self.timeLine.append({"event": "tributeCollected", "seconds": seconds, "team": team})
    ...
    def process_level_event(self, event):
        ...
        if totalEvents > totalTeamEvents:
            ...
            self.timeLine.append({"event": "levelUp", "seconds": seconds, "team": team, "level": level})
    ...
    def process_camp_capture(self, event):
        ...
        if "Boss" in campType:
            ...
            self.timeLine.append({"event": "bossTaken", "seconds": get_seconds_from_int_gameloop(gl), "team": team})
        elif "Bruiser" in campType:
            self.teams[team].generalStats["mercsTaken"] += 1
            self.timeLine.append(
                {"event": "bruiserCampTaken", "seconds": get_seconds_from_int_gameloop(gl), "team": team}
            )
        elif "Siege" in campType:
            self.teams[team].generalStats["siegeCampTaken"] += 1
            self.timeLine.append(
                {"event": "siegeCampTaken", "seconds": get_seconds_from_int_gameloop(gl), "team": team}
            )
                ...
                if self.utpe.get(event["_gameloop"]):
                    ...
                    if len(abilities) > 0:
                        ...
                        if ability:
                            ...
                            self.unitsInGame[ability.targetUnitTag].positions[seconds] = [
                                int(ability.x),
                                int(ability.y),
                            ]
