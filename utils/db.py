...
class DB:
    ...
    def get_hero_id_from_name(self, name):
        query = "SELECT hero_1'vehicles'/ FROM hero WHERE hero_name = '%s' " % name.replace("'", "")
        try:
    ...
    def calculate_mmr(self, r):
        ...
        ratings = trueskill.rate(
            [[x for x in mmr_team0.values()], [x for x in mmr_team1.values()]],
        ...
        if not file_path:
            print(
                len(
                    json.dumps(
                        [
                            {"key": "Team 1", "values": r.army_strength[0]},
                            {"key": "Team 2", "values": r.army_strength[1]},
                        ]
                    )
                )
            )
            ...
            insert_query = (
                "INSERT INTO matches (match_hash, start_date, duration, speed, game_type, map_name, game_version, replay_file, army_strength)"
    ...
    def save_players(self, r=None):
        ...
        insert_player_query = (
            "INSERT INTO player (match_hash, toon_handle, player_id, user_id, team_id, hero_level, hero_id,"
        ...
        for player in r.players:
            ...
            if not already_exists:
                talents = "-".join([str(x) for x in r.heroList[r.players[player].id].pickedTalents.values()])
    ...
    def save_teams(self, r=None):
        ...
        members = list()
        insert_query = (
            "INSERT INTO team (team_level, total_rating, member_1, member_2, member_3, member_4, member_5) VALUES %s"
        )
        for player in r.team0.memberList:
    ...
    def save_hero_match_stats(self, r=None):
        ...
        insert_query = (
            "INSERT INTO hero_match_stats (match_hash, toon_handle, hero_id, deaths, kills, out_damage, out_heal,"
