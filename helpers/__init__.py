...
def get_unit_owners(e, unitsInGame, totalDuration):
    ...
    if e["_event"] == "NNet.Replay.Tracker.SUnitOwnerChangeEvent":
        ...
        if unit.is_sky_temple_tower() or unit.is_dragon_statue() or unit.is_ghostship() or unit.is_braxis_antenna():
            if e["m_upkeepPlayerId"] in (11, 12, 0):
                owner = e["m_upkeepPlayerId"] - 11
            elif e["m_upkeepPlayerId"] in range(1, 6):  # If the player is in the slots 1 to 5, then team 0
                owner = 0
            elif e["m_upkeepPlayerId"] in range(6, 11):  # If the player is in the slots 6 to 10, then team 1
                owner = 1
        ...
        if e["_event"] == "NNet.Replay.Tracker.SUnitOwnerChangeEvent" and e["m_upkeepPlayerId"] in range(0, 11):
            unitTag = get_unit_tag(e)
            ...
            if unit.is_plant_vehicle():
                ...
                not unit.is_sky_temple_tower()
                and not unit.is_plant_vehicle()
                and not unit.is_ghostship()
                and not unit.is_dragon_statue()
            ):
                owner = e["m_upkeepPlayerId"] - 1
...
def get_position_by_second(unit, total_time, start=None, end=None):
    ...
    for second in range(start_game, end_game + 1):
...
def get_unit_clicked(e, unitsInGame):
    ...
    if e["_event"] == "NNet.Game.SCmdUpdateTargetUnitEvent":
        ...
        if unitTag in list(unitsInGame.keys()):
